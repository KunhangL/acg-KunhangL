  ...
  for (unsigned int iw = 0; iw < img_width; ++iw) {
    for (unsigned int ih = 0; ih < img_height; ++ih) {
      ...
      for (int isample = 0; isample < nsample; ++isample) {
        ...
        float rad = hit1_rad * hit0_brdf * hit0_refl.dot(hit0_normal) / (hit0_pdf * nsample);
      ...
      for (int isample = 0; isample < num_half_sample; ++isample) {
        ...
        float rad = w_light * hit1_rad * hit0_brdf * hit0_refl.dot(hit0_normal) / (hit0_pdf_light_sample * nsample);
      ...
      for (int isample = 0; isample < nsample / 2; ++isample) {
        ...
        float rad = w_brdf * hit1_rad * hit0_brdf * hit0_refl.dot(hit0_normal) / (hit0_pdf_brdf_sample * nsample);
        img_mis[ih * img_width + iw] += rad;
      }
    }
  }
